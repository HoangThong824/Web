        ...
        <form method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:border-primary transition-all"><?= $edit_data['description'] ?></textarea>
            </div>
            <button type="submit"
                class="w-full bg-primary hover:bg-primary-dark text-white font-bold py-4 rounded-xl transition-all shadow-lg shadow-primary/30 transform hover:-translate-y-1">
                Lưu danh mục
            </button>
            <?php if ($edit_data['id']): ?>
                <a href="categories.php" class="block text-center text-sm text-slate-400 hover:text-secondary">Hủy chỉnh
                    sửa</a>
            <?php endif; ?>
        </form>
    </div>
